"""
FAIRY 身份 — FOUNDER → FAIRY DID 派生

FOUNDER 是根身份（高权限），FAIRY 是从属身份（桌面宠物）。
...
class FairyIdentity:
    ...
    def __init__(self, founder_did: Optional[str] = None, salt: Optional[str] = None):
        ...
        self.salt = salt or os.getenv("FAIRY_SALT", "fairy_desktop_pet_v3")
    ...
    def from_aid_dir(cls) -> "FairyIdentity":
        """
        从 AID 目录加载 FOUNDER 身份并派生 FAIRY DID
    ...
    def _derive_did(self) -> str:
        ...
        if not self.founder_did:
            ...
            return f"{FAIRY_DID_PREFIX}:anonymous:{random_bytes[:16].hex()}"

        key = self.founder_did.encode("utf-8")
        msg = f"fairy{self.salt}".encode("utf-8")
        digest = hmac.new(key, msg, hashlib.sha512).hexdigest()
        return f"{FAIRY_DID_PREFIX}:{self.founder_did.split(':')[-1][:8]}:{digest[:16]}"
    ...
    def verify(self, other_did: str) -> bool:
        ...
        msg = f"fairy{self.salt}".encode("utf-8")
        digest = hmac.new(key, msg, hashlib.sha512).hexdigest()
        expected = f"{FAIRY_DID_PREFIX}:{self.founder_did.split(':')[-1][:8]}:{digest[:16]}"
